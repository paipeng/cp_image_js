    ...
    rotate(mat, degree) {
        ...
        for (i = 0; i < dim1; i++) {
            for (j = 0; j < dim2; j++) {
                ...
                if (crdy < 0)
                    fy = parseInt(crdy.toFixed(19)) - 1;
                else
                    fy = parseInt(crdy.toFixed(19)); // floor-alternative
                if (crdx < 0)
                    fx = parseInt(crdx.toFixed(19)) - 1;
                else
                    fx = parseInt(crdx.toFixed(19)); // floor-alternative

                a = crdy - fy; // y-weights
                b = crdx - fx; // x-weights


                x = parseInt(lx + dx + fx + 1);
                y = parseInt(ly + dy + fy + 1);
