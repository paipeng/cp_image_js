...
class CPHarrisCorner {
    ...
    findCorners(mat, endImgType) {
        ...
        while (n--) {
            ...
            if (mat.channel > 1) {
                input.data[mat.channel * n] =
                    input.data[mat.channel * n + 1] =
                    input.data[mat.channel * n + 2] = (input.data[mat.channel * n] + input.data[mat.channel * n + 1] + input.data[mat.channel * n + 2]) / 3;


            }
            if (n % wid > 0)
                ix[n] = (input.data[mat.channel * n] - input.data[mat.channel * (n - 1)]);

            if (n >= wid)
                iy[n] = (input.data[mat.channel * n] - input.data[mat.channel * (n - wid)]);
        }
        ...
        if (endImgType == 0) ctx.putImageData(input, 0, 0);
            ...
            while (n--) {
                input.data[mat.channel * n + 0] = Math.max(Math.min(Math.round(intScale * result[n]) + 128, 255), 0);
                if (mat.channel > 1) {
                    input.data[mat.channel * n + 1] = Math.max(Math.min(Math.round(intScale * result[n]) + 128, 255), 0);
                    input.data[mat.channel * n + 2] = Math.max(Math.min(Math.round(intScale * result[n]) + 128, 255), 0);
                }
                if (mat.channel == 4) {
                    input.data[mat.channel * n + 3] = 255;
