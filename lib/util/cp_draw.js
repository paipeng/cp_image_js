...
class CPDraw {
    ...
    drawLine(mat, line, color = 0x00) {
        ...
        if (line.start.x == line.end.x && line.start.y == line.end.y) {
            mat.data[line.start.y * mat.width * mat.channel + line.start.x * mat.channel] = 0x00;
            if (mat.channel > 1) {
                mat.data[line.start.y * mat.width * mat.channel + line.start.x * mat.channel + 1] = 0x00;
                mat.data[line.start.y * mat.width * mat.channel + line.start.x * mat.channel + 2] = 0x00;
            }
            return;
        }

        mat.data[line.start.y * mat.width * mat.channel + line.start.x * mat.channel] = 0x00;
        mat.data[line.end.y * mat.width * mat.channel + line.end.x * mat.channel] = 0x00;

        if (min_point.x == max_point.x) {
            for (var i = min_point.y + 1; i < max_point.y; i++) {
                mat.data[i * mat.width * mat.channel + line.end.x * mat.channel] = 0x00;
                if (mat.channel > 1) {
                    mat.data[i * mat.width * mat.channel + line.end.x * mat.channel + 1] = 0x00;
                    mat.data[i * mat.width * mat.channel + line.end.x * mat.channel + 2] = 0x00;
                }
            }
        } else if (min_point.y == max_point.y) {
            for (var j = min_point.x + 1; j < max_point.x; j++) {
                mat.data[line.end.y * mat.width * mat.channel + j * mat.channel] = 0x00;
                if (mat.channel > 1) {
                    mat.data[line.end.y * mat.width * mat.channel + j * mat.channel + 1] = 0x00;
                    mat.data[line.end.y * mat.width * mat.channel + j * mat.channel + 2] = 0x00;
                }
            }
        } else {
            var offset = 0;
    ...
    drawPolygon(mat, points, color) {
        ...
        for (var i in points) {
            i = parseInt(i);
            this.drawLine(mat, new CPLine(points[i], points[(i + 1) % len]));
        }
    }
