        ...
        if (min_point.x == max_point.x) {
            ...
            if (Math.abs(k) < 1) {
                for (var j = min_point.x + 1; j < max_point.x; j++) {
                    mat.data[min_point.y + parseInt((j - min_point.x) * k + offset) * line.start.x * mat.channel + j * mat.channel] = 0x00;
                    if (mat.channel > 1) {
                        mat.data[min_point.y + parseInt((j - min_point.x) * k + offset) * line.start.x * mat.channel + j * mat.channel + 1] = 0x00;
                        mat.data[min_point.y + parseInt((j - min_point.x) * k + offset) * line.start.x * mat.channel + j * mat.channel + 2] = 0x00;
                    }
                }
            } else {
                if (min_point.y < max_point.y) {
                    for (j = min_point.y + 1; j < max_point.y; j++) {
                        mat.data[j * line.start.x * mat.channel + (min_point.x + 1 + parseInt((j - min_point.y) / k - offset)) * mat.channel] = 0x00;
                        if (mat.channel > 1) {
                            mat.data[j * line.start.x * mat.channel + (min_point.x + 1 + parseInt((j - min_point.y) / k - offset)) * mat.channel + 1] = 0x00;
                            mat.data[j * line.start.x * mat.channel + (min_point.x + 1 + parseInt((j - min_point.y) / k - offset)) * mat.channel + 2] = 0x00;
                        }
                    }
                } else {
                    for (j = max_point.y + 1; j < min_point.y; j++) {
                        mat.data[j * line.start.x * mat.channel + (max_point.x - 1 + parseInt((j - max_point.y) / k - offset)) * mat.channel] = 0x00;
                        if (mat.channel > 1) {
                            mat.data[j * line.start.x * mat.channel + (max_point.x - 1 + parseInt((j - max_point.y) / k - offset)) * mat.channel + 1] = 0x00;
                            mat.data[j * line.start.x * mat.channel + (max_point.x - 1 + parseInt((j - max_point.y) / k - offset)) * mat.channel + 2] = 0x00;
                        }
                    }
                }
            }
